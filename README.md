...
Each member has their own file in `_people/`. To update yours, no Git CLI needed — everything works in the browser via github.com.

### 1. Edit your card

Open your file (e.g. [`_people/tea-kocijan.qmd`](https://github.com/TizzuLab/zindellab/tree/main/_people)) and click the **pencil icon** top-right. Change the name, role, room, email, phone, or bio. Scroll to the bottom → **Commit changes** → leave the default options, commit directly to `main`.

The site rebuilds within ~1–2 minutes. You can watch it at [github.com/TizzuLab/zindellab/actions](https://github.com/TizzuLab/zindellab/actions).
...
3. **Filename must match your card slug**, e.g. `tea-kocijan.jpg`.
...
Replace `YOUR-HANDLE` with your own. The icon shows automatically because Bootstrap Icons are loaded site-wide. See Joel's card for a live example.

For other profiles (ORCID, Google Scholar, Twitter/X, GitHub, Bluesky) use the matching `bi-...` class — list at [icons.getbootstrap.com](https://icons.getbootstrap.com), search for the brand name.

### 4. Optional — advertise a semester project

If you have an open topic you'd like a semester student to work on, feel free to mention it in your bio. A sentence or two is enough. Example:

> *I work on macrophage clustering at peritoneal injuries. **Open for semester projects:** a project on calcium-dependent aggregation in MARCO-knockout mice — drop me an email if interested.*
...
If anything else needs changing on the site, ping Joel.
...
If a commit accidentally breaks the build (rare — typically a YAML or markdown typo), the site stays on the last working version. Nothing broken ever goes public; Joel gets an email about the failed build.

## Tech notes

- Framework: [Quarto](https://quarto.org) (multi-page website mode)
- Theme: dark-only, custom SCSS layer on top of `darkly`
- Font: DIN Next (ETH webfont license), self-hosted under `assets/fonts/`
- Deploy: GitHub Pages via `.github/workflows/publish.yml` using the modern Actions flow
- Custom domain: `zindellab.ch` via the `CNAME` file at repo root
